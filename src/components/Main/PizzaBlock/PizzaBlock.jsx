...
const PizzaBlock = ({id, name, price, imageUrl,types,sizes, setSortAction,countPizza}) => {

    const typePizza = ["тонкое", "традиционное"]
    const typeSize  = [26, 30, 40];

    const [activeType, setActiveType] = useState(types[0])
    const [activeSize, setActiveSize] = useState(typeSize.findIndex(el => el == sizes[0]))

  function onClickAddPizza(){
      setSortAction({id, name, price, imageUrl,size:typeSize[activeSize],type:typePizza[activeType]})
  }
